...
import com.ea.core.cache.redis.pool.RedisShardedPool;
...
public abstract class AbstractCacheHandle implements ICacheHandle, ApplicationContextAware {
	...
	public AbstractCacheHandle(String cacheLevel) {
		...
		if (activate) {
			...
			cachePool.addServer(CacheDefinition.getPropertyValue(cacheLevel + "servers"));
	...
	/**
	 * 迭代查找所有Level缓存器里符合正则的Key(不同Level缓存器里存在相同Key时，只返回一个)
	 */
	public Set<String> keys(String pattern, String regexp) throws Exception{
		...
		this.setNextHandle();
		if (this.nextHandle != null) {
			Set<String> set = this.nextHandle.keys(pattern, regexp);
			for(String key : set){
	...
	public Map<String, String> getByRegexp(String pattern, String regexp) throws Exception{
		...
		this.setNextHandle();
		if (this.nextHandle != null) {
			Map<String, String> map = this.nextHandle.getByRegexp(pattern, regexp);
			mapVal.putAll(map);
		}
		return mapVal;
	}
